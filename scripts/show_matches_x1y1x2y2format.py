...
def Lines(img1_path, img2_path, matches_path) -> None:
    ...
    ax.matshow(
        merged_img,
        cmap="gray",
        vmin=min(img1_low, img2_low),
        vmax=max(img1_high, img2_high),
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--matches", type=str, required=True, help="Path to the matches file."
    )
    parser.add_argument(
        "--method",
        type=str,
        required=False,
        default="default",
        help="Method to use for matching 'points' or 'lines'",
    )
