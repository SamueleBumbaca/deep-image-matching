...
for key in matches_dict:
    ...
    with open("{}/{}.txt".format(kpts_dir, img1), "r") as kp1_file, open(
        "{}/{}.txt".format(kpts_dir, img2), "r"
    ) as kp2_file, open(
        "{}\matches_no_static.txt".format(out_dir), "a"
    ) as out_matches_file:
        ...
        for item in matches_dict[key]:
            ...
            if (
                flux_x > STATIC_THRESHOLD_X_MIN
                and flux_x < STATIC_THRESHOLD_X_MAX
                and flux_y > STATIC_THRESHOLD_Y_MIN
                and flux_y < STATIC_THRESHOLD_Y_MAX
            ):
                # print('ok')
                out_matches_file.write("{} {}\n".format(m1, m2))
