...
def loadJSON():
    ...
    view_ids = {
        view["key"]: view["value"]["ptr_wrapper"]["data"]["filename"]
...
def loadFeatures(filename):
    keyp_filename = os.path.join(
        args.matches, f"{os.path.splitext(filename)[0]}.keyp.pt"
    )
    desc_filename = os.path.join(
        args.matches, f"{os.path.splitext(filename)[0]}.desc.pt"
    )
    scor_filename = os.path.join(
        args.matches, f"{os.path.splitext(filename)[0]}.scor.pt"
    )
...
def featureExtraction():
    ...
    for image_path in tqdm(image_paths):
        ...
        if width % 2 != 0 or height % 2 != 0:
            print("Img size should be divisible by 2. Exit().")
            quit()
        ...
        threading.Thread(
            target=lambda: saveFeatures(
                keypoints, features.descriptors, features.detection_scores, basename
            )
        ).start()

        threading.Thread(
            target=lambda: saveFeaturesOpenMVG(basename, keypoints)
        ).start()
        threading.Thread(
            target=lambda: saveDescriptorsOpenMVG(basename, features.descriptors)
        ).start()
...
def featureMatching():
    ...
    putative_matches = []
    for image1_index, image2_index in tqdm(
        (
            np.loadtxt(args.pair_list, dtype=np.int32)
            if args.pair_list != None
            else np.asarray([*combinations(view_ids, 2)], dtype=np.int32)
        )
    ):
        ...
        lafs1 = KF.laf_from_center_scale_ori(
            keyp1[None], 96 * torch.ones(1, len(keyp1), 1, 1, device=device)
        )
        lafs2 = KF.laf_from_center_scale_ori(
            keyp2[None], 96 * torch.ones(1, len(keyp2), 1, 1, device=device)
        )

        dists, idxs = lightglue(desc1, desc2, lafs1, lafs2)

        putative_matches.append(
            [image1_index, image2_index, idxs.cpu().numpy().astype(np.int32)]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--matches", type=str, required=True, help="Path to the matches directory"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="LightGlue early stopping (-1 - disable)",
    )
    parser.add_argument(
        "--width_confidence",
        type=float,
        default=0.99,
        help="LightGlue point pruning (-1 - disable)",
    )
    parser.add_argument(
        "--filter_threshold", type=float, default=0.99, help="LightGlue match threshold"
    )
    ...
    device = torch.device(
        "cuda"
    )  # if args.force_cpu else K.utils.get_cuda_device_if_available()
