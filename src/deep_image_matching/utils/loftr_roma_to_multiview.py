...
def add_keypoints(db, h5_path, image_path, camera_model, single_camera=True):
    keypoints_path = os.path.join(h5_path, "keypoints.h5")
    with h5py.File(keypoints_path, "r") as keypoint_f:
        fname_to_id = {}
...
def add_matches(db, h5_path, fname_to_id):
    ...
    matches_path = os.path.join(h5_path, "matches.h5")
            ...
            for key_1 in match_file.keys():
                ...
                for key_2 in group.keys():
                    ...
                    if pair_id in added:
                        ...
                        continue
                    matches = group[key_2][()]
                    # db.add_matches(id_1, id_2, matches) # comment if any error
...
def import_into_colmap(img_dir, feature_dir=".featureout", database_path="database.db"):
    ...
    with COLMAPDatabase.connect(database_path) as db:
        ...
        fname_to_id = add_keypoints(
            db, feature_dir, img_dir, "simple-radial", single_camera
        )
        add_matches(
            db,
            feature_dir,
...
def LoftrRomaToMultiview(
    ...
    if mask_dir:
        ...
        mask_paths = {
            p: os.path.join(mask_dir, p.replace(img_ext, ".png")) for p in image_names
        }
        mask_arr = {}
        for p in image_names:
            if not os.path.exists(mask_paths[p]):
                FileNotFoundError(
                    f"Mask for {p} not found, expected path: {mask_paths[p]}"
                )
            mask_arr[p] = np.array(Image.open(mask_paths[p])) > 0  # ensure binary
    with h5py.File(rf"{input_dir}/features.h5", mode="r") as h5_feats, h5py.File(
        rf"{input_dir}/matches.h5", mode="r"
    ) as h5_matches, h5py.File(rf"{input_dir}/matches_loftr.h5", mode="w") as h5_out:
    ...
    with h5py.File(rf"{input_dir}/matches_loftr.h5", mode="r") as f_match:
        for k1 in f_match.keys():
            ...
            for k2 in group.keys():
                matches = group[k2][...]
                if mask_dir:
                    y1, x1 = matches[:, 1].astype(int), matches[:, 0].astype(int)
                    y2, x2 = matches[:, 3].astype(int), matches[:, 2].astype(int)
                    valid_mask = (
                        (y1 < mask_arr[k1].shape[0])
                        & (x1 < mask_arr[k1].shape[1])
                        & (y2 < mask_arr[k2].shape[0])
                        & (x2 < mask_arr[k2].shape[1])
                    )
                    # Filter matches using the boolean mask
                    matches = matches[valid_mask]
                    # Update matches with the filtered matches
                    matches = matches[
                        mask_arr[k1][
                            matches[:, 1].astype(int), matches[:, 0].astype(int)
                        ]
                        & mask_arr[k2][
                            matches[:, 3].astype(int), matches[:, 2].astype(int)
                        ]
                    ]
                # Avoid pairs with no matches
                if matches.shape[0] != 0:
                    ...
                    total_kpts[k1]
                    kpts[k1].append(torch.empty((0, 2), dtype=torch.int))
                    kpts[k2].append(torch.empty((0, 2), dtype=torch.int))
                    match_indexes[k1][k2] = torch.empty((0, 2), dtype=torch.int)
    for k in kpts.keys():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--mask_dir", type=str, default=None, help="Mask directory"
    )
    ...
    LoftrRomaToMultiview(input_dir, output_dir, image_dir, img_ext, mask_dir)
