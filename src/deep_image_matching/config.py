...
class Config:
    ...
    def parse_general_config(input_args: dict) -> dict:
        ...
        if args["mask"] is not None:
            ...
            if not args["mask"].exists() or not args["mask"].is_dir():
                Warning(
                    f"Invalid mask folder {args['mask']}. Trying to find mask folder in project folder."
                )
                args["mask"] = args["dir"] / "masks"
                if not args["mask"].exists() or not args["mask"].is_dir():
                    raise ValueError(
                        f"Invalid mask folder {args['mask']}. Direcotry does not exist"
                    )
